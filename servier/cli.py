...
from .config import (DATA_PATH, RAW_DATA_PATH, PRIMARY_DATA_PATH, MODEL_DATA_PATH, REPORTING_DATA_PATH)
from .config import (INPUT_FILENAME, DATA_TRAIN_FILENAME, DATA_VALID_FILENAME, DATA_TEST_FILENAME, MODEL_DATA_PATH_NAME)
...
def main():
    ...
    if mode == "split":
        ...
        train(input_path, valid_path=valid_path, model_path=model_path, reporting_path=reporting_path)
    elif mode == "evaluate":
        if input_path is None:
...
def split_data(input_path, output_path, test_only):
    print("Split..")
